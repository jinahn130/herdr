...
`idle` and `done` both mean the agent is ready for input. The CLI/API uses the server's seen state: `done` is idle but not yet marked seen, and reads do not mark it seen. Each TUI client tracks viewed completions independently, so a client's Done badge can differ from the CLI or another client's badge. Focus actions acknowledge completion according to `ui.done_acknowledgement`: the default `"tab"` acknowledges every completed pane in the focused tab for that client, while `"pane"` acknowledges only the explicitly focused pane. With pane acknowledgement, merely returning focus to the outer Herdr window does not acknowledge any pane. Explicit `pane focus` / `agent focus` commands follow the configured scope. `blocked` means Herdr recognized an approval or question UI. `unknown` means an agent is present but Herdr cannot classify its lifecycle confidently; it does not prove successful completion. Use exact `--until` states when that distinction matters.
