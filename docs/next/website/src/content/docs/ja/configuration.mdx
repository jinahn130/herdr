...
デフォルトでは、Herdr にフォーカスするとアクティブなタブ内で完了したすべてのエージェントが表示済みとなり、状態が `done` から `idle` に変わります。他の完了済みペインを `done` のまま残し、フォーカスしたペインだけを表示済みにするには、次のように設定します:
...
ペイン単位の表示済み設定では、外側の Herdr ウィンドウにフォーカスを戻すだけでは、どのペインも表示済みになりません。ペインをクリックする、フォーカスをそのペインへ移動する、または `pane focus` / `agent focus` で指定すると、そのペインだけが表示済みになります。
