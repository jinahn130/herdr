...
`idle` と `done` はどちらも入力可能な状態です。CLI/API はサーバーの既読状態を使い、未確認の idle を `done` とします。読み取りだけでは既読になりません。各 TUI クライアントは表示済みの完了を独立して管理するため、Done バッジが CLI やほかのクライアントと異なる場合があります。フォーカス操作で完了を表示済みにする範囲は `ui.done_acknowledgement` に従います。デフォルトの `"tab"` では、そのクライアントでフォーカスしたタブ内の完了済みペインがすべて表示済みになります。`"pane"` では、明示的にフォーカスしたペインだけが表示済みになります。ペイン単位の場合、外側の Herdr ウィンドウにフォーカスを戻すだけでは、どのペインも表示済みになりません。明示的な `pane focus` / `agent focus` コマンドも設定された範囲に従います。`blocked` は承認または質問 UI を Herdr が認識した状態です。`unknown` はエージェントが存在するもののライフサイクルを確実に分類できない状態で、成功完了を意味しません。違いが重要なら正確な `--until` を指定してください。
