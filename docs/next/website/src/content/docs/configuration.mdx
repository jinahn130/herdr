...
By default, focusing Herdr acknowledges every completed agent in the active tab, changing its status from `done` to `idle`. To acknowledge only the pane you focus while leaving other completed panes marked `done`, configure:
...
With pane acknowledgement, merely returning focus to the outer Herdr window does not acknowledge any pane. Clicking a pane, navigating focus to it, or targeting it with `pane focus` / `agent focus` acknowledges that pane only.
