...
默认情况下，聚焦 Herdr 会确认活动标签页中所有已完成的智能体，并将其状态从 `done` 改为 `idle`。若要只确认你聚焦的窗格，同时让其他已完成窗格保持 `done`，请配置：
...
使用窗格级确认后，仅将焦点切回 Herdr 外层窗口不会确认任何窗格。点击窗格、将焦点导航到窗格，或通过 `pane focus` / `agent focus` 指定窗格，只会确认该窗格。
