...
`idle` 和 `done` 都表示可以接受输入。CLI/API 使用服务器的已查看状态：`done` 表示尚未标记为已查看的 idle，读取不会将其标记为已查看。每个 TUI 客户端独立记录它已查看的完成状态，因此其 Done 标记可能与 CLI 或其他客户端不同。聚焦操作会按照 `ui.done_acknowledgement` 的设置确认完成状态：默认的 `"tab"` 会为当前客户端确认聚焦标签页内所有已完成的窗格；`"pane"` 则只确认显式聚焦的窗格。使用窗格级确认时，仅将焦点切回 Herdr 外层窗口不会确认任何窗格。显式 `pane focus` / `agent focus` 命令也遵循配置的范围。`blocked` 表示 Herdr 识别到了审批或提问界面。`unknown` 表示智能体存在,但 Herdr 无法可靠判断其生命周期;它不代表工作成功完成。区别重要时,请指定精确的 `--until` 状态。
